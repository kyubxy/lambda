...
happyExpList = Happy_Data_Array.listArray (0,30) ([704,16392,9216,0,0,11264,1600,0,11265,0,0,0
	])

{-# NOINLINE happyExpListPerState #-}
happyExpListPerState st =
    token_strs_expected
  where token_strs = ["error","%dummy","%start_parse","Expr","App","Simple","vars","'/'","'.'","'('","')'","%eof"]
        bit_start = st * 12
        bit_end = (st + 1) * 12
...
happyReduction_1 ((HappyAbsSyn4  happy_var_4) `HappyStk`
	_ `HappyStk`
	(HappyTerminal (TokenVars happy_var_2)) `HappyStk`
	_ `HappyStk`
	happyRest)
	 = HappyAbsSyn4
		 (PtAbstract happy_var_2 happy_var_4
	) `HappyStk` happyRest
...
happyReduction_3 (HappyAbsSyn6  happy_var_2)
	(HappyAbsSyn4  happy_var_1)
	 =  HappyAbsSyn5
		 (PtApply happy_var_1 happy_var_2
	)
...
happyReduction_4 (HappyAbsSyn6  happy_var_1)
	 =  HappyAbsSyn5
		 (PtSimple happy_var_1
	)
happyReduction_4 _  = notHappyAtAll 

happyReduce_5 = happySpecReduce_1  6 happyReduction_5
happyReduction_5 (HappyTerminal (TokenVars happy_var_1))
	 =  HappyAbsSyn6
		 (PtVar happy_var_1
	)
...
happyNewToken action sts stk (tk:tks) =
	...
	case tk of {
	TokenVars happy_dollar_dollar -> cont 7;
	TokenLambda -> cont 8;
	TokenDot -> cont 9;
	TokenLParen -> cont 10;
	TokenRParen -> cont 11;
